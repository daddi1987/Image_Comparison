...
#------------------------VAR FROM GUI----------------------------------------------------------
PathImageMasterGUI = source_master.get()
PathImageSecond = source_file.get()
PathFolderLogGUI = source_log.get()
NumberFilesToCompareGUI = int(E4.get())
ControlCheck = bool(Cutter_Check.get())
ResolutionControl = float(E5.get())

#---------------------SPLIT PATH---------------------------------------------------------------
print("-----------------------VAR FROM GUI-------------------------")
print(PathImageMasterGUI)
print(PathImageSecond)
print(PathFolderLogGUI)
print(NumberFilesToCompareGUI)
print(ControlCheck)
print(ResolutionControl)

PathImageMaster = os.path.dirname(PathImageMasterGUI)
NameFile_Image = os.path.basename(PathImageMasterGUI)
PathImageSlave = os.path.dirname(PathImageSecond)
NameFile_ImageS = os.path.basename(PathImageSecond)

#PathImageMaster = os.path.dirname(args["first"])
#NameFile_Image = os.path.basename(args["first"])
NameFile_ImageP = os.path.splitext(NameFile_ImageS)[0]
NameFile_ImageP = NameFile_ImageP[:9]
extantionFile = os.path.splitext(NameFile_ImageS)[1]
Resize = ControlCheck
#-----------------------VAR TO BLOCK---------------------------------------------------------
block = 0;
bufferFileNotFound = 10
targetscore = ResolutionControl

#----------------------CREATE A FILE TXT LOG--------------------------------------------------
path_folder = PathFolderLogGUI
...
#-------------------FOR NUMBER PHOTO-----------------------------------------------------------
ActualPhoto = 0
NumberPhoto = NumberFilesToCompareGUI
#print(ActualPhoto)
print(NumberPhoto)
...
while (ActualPhoto != (NumberPhoto + bufferFileNotFound)):
   ...
   # print(NameFile_ImageP)
    ...
    #print(PathImageSecond)
    # ---------------------------------------------------------------------------------------------
    PhotoAnalysis = os.path.join(PathImageSlave, NameFile_Image)
    print(PhotoAnalysis)
    #print(incrementPhoto)
    if os.path.exists(PhotoAnalysis):
        ...
        if Resize == True and block == 0:
            ...
            if len(refPt):
                ...
                imageCheck = cv2.imread(PathImageMasterGUI)
            ...
            imageA = cv2.imread(PathImageMasterGUI)
        ...
        if score >= targetscore:
            ...
            LogFile.write("\n")
            LogFile.write(NameFile_Image + ":  matching image" + ";" + str(score) + ";" + str(targetscore) + ";" + "0")
            LogFile.close()
            ...
            pathFile_Image = os.path.basename(PathImageSecond)
            ...
            try:
                ...
                LogFile.write("\n")
                LogFile.write(NameFile_Image + ":  not matching image" + ";" + str(score) + ";" + str(targetscore) + ";" + "1")
                LogFile.close()
                print("LOG",NameFile_Image)
                # ----------END LOG---------------------------------------------
    ...
    Image_GUI.messagebox.showinfo("Finish","Completed Successfully")
